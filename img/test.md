...
thetaResult = rad2deg(  theta(max(b)) );
	...
	1. 复制项目到E盘
	2. 安装VS
	3. 运行setup.exe进入installer界面
	4. 选择安装选项
	5. 选择：主要使用C++的桌面开发 以及其他组件  ALT,SDK
	
	6. 重启运行，修改个项目属性
	7. 会出现AudiotestTool  有jni.h文件无法打开，这是因为audiotestool属性里面的参数没设置好。选择包含目录里面有JAVA/jdk的路径
	8. 安装JAVA JDK.exe 然后把包含目录里面对应的Include和win32路径改成新安装后的对应路径
	9. 运行release，可能会出现代码红波浪，一般是编辑器的问题
	原因是：推荐使用方法2
	在代码编写的过程中，VS报错：“应输入声明”。
	比如在“if”或者一个已经定义好的变量下面有错误的红线出现，鼠标挪上去之后，就会显示“应输入声明”
	1、前面有用“//”注释信息
	在一行注释的最后有一个右斜杠，有可能系统将他识别为换行符之后，然后将下一句的"int a = 1"也理解为注释信息了。
	解决办法：将注释行使用“/*....*/”来注释
	
	2、如果上面不行，试试这个解决办法：
	“工具”----->“选项”----->“C/C++”----->“高级”----->“禁用自动更新”，将禁用自动更新改成：“True”。
	10. 暂时没有其他问题，release程序运行通过
